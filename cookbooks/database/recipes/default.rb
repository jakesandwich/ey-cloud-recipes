...
#
	...
	node[:engineyard][:environment][:apps].each do |app|
		...
		#if app[:name] == 'home_production' || app[:name] == 'home_production_clone'
		# create new database.yml
		 template "/data/#{app[:name]}/shared/config/database.yml" do
			...
			variables({
									...
									:second_connection => 'cms_production',
									:second_database => 'cms_production',

									:third_connection => 'enterprise_production',
									:third_database => 'enterprise'
								})
		 end
		#end
	end
